...
const LOCAL_APP_DIR = process.env.CODEXMOBILE_CONFIG_DIR || path.join(ROOT_DIR, '.codexmobile');
const STATE_DIR = process.env.CODEXMOBILE_HOME || path.join(ROOT_DIR, '.codexmobile', 'state');
const UPLOAD_ROOT = process.env.CODEXMOBILE_UPLOAD_ROOT || path.join(ROOT_DIR, '.codexmobile', 'uploads');
const IMAGE_PROMPT_STATE = path.join(STATE_DIR, 'image-prompts.json');
const FEISHU_AUTH_STATE = path.join(STATE_DIR, 'feishu-auth.json');
const PUSH_STATE = path.join(STATE_DIR, 'push-notifications.json');
const PORT = Number(process.env.PORT || 3321);
const HOST = process.env.HOST || '0.0.0.0';
const HTTPS_PORT = Number(process.env.HTTPS_PORT || 3443);
const HTTPS_PFX_PATH = process.env.HTTPS_PFX_PATH || path.join(LOCAL_APP_DIR, 'tls', 'server.pfx');
const HTTPS_ROOT_CA_PATH = process.env.HTTPS_ROOT_CA_PATH || path.join(LOCAL_APP_DIR, 'tls', 'codexmobile-root-ca.cer');
